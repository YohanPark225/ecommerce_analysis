...
sessions <- read.csv("mock_sessions_v2.csv", stringsAsFactors = FALSE)

transactions <- read.csv("mock_transactions_v2.csv", stringsAsFactors = FALSE)

ad_campaigns <- read.csv("mock_ad_campaign_v2.csv", stringsAsFactors = FALSE)
...
user_purchase_count_02 %>% summarise(total_rows = n())
...
## Let's use the 'user_purchase_count' data frame again, but this time we create the table from 'transactions' dataset.
...
## As a follow-up question, you should ask if they're statistically different, or not.

# Question 03: Let's break down the "repeat" customers. I mean, what about spending per purchase by 3, 4, or 5 timers? 
...
length(unique(sessions_freq$user_id[sessions_freq$purchase_freq=="repeat"])) # confirm the number of repeat customer is 841 as we know it
...
## (1) (Three) Pie Charts to show medium distribution for "never", "one-time", and "repeat" customers?
...
### Now, I can do the same for the sources and devices...  
...
class(transactions_btw_purchase$date)
